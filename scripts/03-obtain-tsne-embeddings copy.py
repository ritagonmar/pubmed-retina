...
# save
sp.sparse.save_npz(variables_path / "affinities_2025", A.P)
end = time.time()
print("Runtime Affinities: ", str(timedelta(seconds=end - start)))

## initialization
I = initialization.pca(embedding_sep_all, random_state=42)
np.save(variables_path / "initialization_2025", I)
end = time.time()
print("Runtime Initialization: ", str(timedelta(seconds=end - start)))

# ## load stuff
# # load affinities P
# affinities_P_bert_reparsed = sp.sparse.load_npz( variables_path / "affinities_2025.npz")
